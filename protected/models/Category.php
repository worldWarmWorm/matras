...
class Category extends \common\components\base\ActiveRecord
{
    ...
    public function behaviors()
    {
        return A::m(parent::behaviors(), array(
            ...
            'offerAttributesBehavior'=>[
                'class'=>'\common\ext\dataAttribute\behaviors\DataAttributeBehavior',
                'attribute'=>'offer_attributes',
                'attributeLabel'=>'Атрибуты для торговых предложений'
            ]
        ));
    }
    ...
    /**
     * Получить атрибуты торговых предложений
     * @return array
     */
    public function getOfferAttributes($returnListData=false)
    {
        $data=[];
        if(!$this->isRoot()) {
            if($ancestors=$this->ancestors()->findAll(['select'=>'id,offer_attributes,lft,rgt,level,root'])) {
                foreach($ancestors as $ancestor) {
                    $data=$ancestor->offerAttributesBehavior->get(true);
                    if(!empty($data)) {
                        break;
                    }
                }
            ...
            $data=$this->offerAttributesBehavior->get(true);
        }
        
        if($returnListData) {
            $_data=[];
            foreach($data as $item) {
                $_data[$item['hash']]=$item['title'];
            }
            $data=$_data;
        }
    ...
    public function beforeSave()
    {
        ...
        if($data=$this->offerAttributesBehavior->get(true)) {
            foreach($data as $idx=>$item) {
                if(empty($item['hash'])) {
                    $data[$idx]['hash']=HHash::u('h');
                }
            }
            $this->offerAttributesBehavior->set($data);
        }
    ...
    public function getProductAttributes()
    {
        ...
        foreach (array_reverse($ancestors) as $ancestor) {
            if ($ancestor->offerAttributesBehavior->get(true)) {
                return $ancestor->offerAttributesBehavior->get(true);
            }
        }
        
        if ($this->offerAttributesBehavior->get(true)) {
            return $this->offerAttributesBehavior->get(true);
        }
