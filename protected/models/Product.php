...
class Product extends \common\components\base\ActiveRecord
{
    ...
    public function behaviors()
    {
        return A::m(parent::behaviors(), [
            ...
            'complectationsBehavior' => [
                'class' => '\common\behaviors\ARAttributeListBehavior',
                'attribute' => 'complectations',
                'rel' => '\ProductComplectations',
                'addColumn' => false
            ],
            //         	'imageBehavior'=>[
    ...
    public function scopes()
    {
        return $this->getScopes([
            ...
            'cardColumns' => [
                'select' => '`t`.`offers`, `t`.`id`, `t`.`category_id`, title, code, price, sale, new, hit, link_title, notexist, old_price, `main_image`, `main_image_alt`, `main_image_enable`, `hidden`'
            ],
    ...
    public function getRelatedCriteria($categoryId = null, $tableAlias = '`t`')
    {
        ...
        if (is_array($categoryId)) {
            $criteria->join .= ' IN (' . implode(',', array_map(function ($id) {
                return (int)$id;
            }, $categoryId)) . ')';
        } else {
...
*/
    ...
    {
        return count($this->offersBehavior->get(true)) > 0;
    }
    ...
    public function getOffersData()
    {
        ...
        $attributes = $this->category->getOfferAttributes(true);

        $offers = $this->offersBehavior->get(true);
        if (!empty($offers)) {
            foreach ($offers as $idx => $item) {
                $price = (float)$item['price'];
                if (!$price) $price = (float)$this->price;
                elseif ($price < 0) $price = 0;

                $data['offers'][$idx]['price'] = $price;
                foreach ($attributes as $hash => $title) {
                    $hash = trim($hash);
                    $title = preg_replace('/["\']+/', '', trim($title));

                    if (empty($title) || empty($hash)) continue;

                    if (!empty($item[$hash])) {
                        $value = trim($item[$hash]);
                        if (is_numeric($value) || !empty($value)) {
                            $data['offers'][$idx][$title] = $value;
                            $data['props'][$title][$value] = $value;
                        }
                    }
                }

                // наборы свойств (свойство1=>(свойство2..N=>значения))
                foreach ($data['offers'] as $idx => $props) {
                    if (isset($props['price'])) unset($props['price']);
                    $_props = [];
                    foreach ($props as $prop => $value) {
                        if (empty($_props[$prop])) $_props[$prop] = [];
                        $_props[$prop][] = $value;
                    }
                    $props = $_props;
                    foreach ($props as $prop => $values) {
                        foreach ($values as $value) {
                            if (empty($data['sets'][$prop][$value])) {
                                $data['sets'][$prop][$value] = [];
                            }
                            $data['sets'][$prop][$value] = A::m($data['sets'][$prop][$value], $props);
                        }
                    }
                    foreach ($data['sets'] as $prop => $values) {
                        foreach ($values as $value => $props) {
                            foreach ($props as $prop2 => $values2) {
                                $data['sets'][$prop][$value][$prop2] = array_unique($values2);
                            }
                        }
                    }
                }
    ...
    public function afterAddCart(&$item, $attribute, $value)
    {
        ...
        if (!empty($item['attributes']['complectation'])) {
            $product = Product::model()->findByPk($item['id']);
            $offers = $product->getOffersData();
            $complectationString = $offers['offers'][0]['Доп. комплектация'];
            $complectations = explode('|', $complectationString);
            $complectation = [];
            $names = [];
            ...
            foreach ($names as $name) {
                if (strstr($item['attributes']['complectation'], $name)) {
                    $selectedComplectations[] = $name;
                } else {
                    continue;
                }
            }
            ...
            foreach ($complectations as $complectation) {
                if (strstr($complectation, $selectedComplectations[$j])) {
                    $costs[] = explode('-', $complectation);
                } else {
                    continue;
                }
                $j++;
        ...
        if (!empty($item['attributes']['offer'])) {
            if (is_string($item['attributes']['offer'])) {
                $offer = @json_decode($item['attributes']['offer'], true);
                if (is_array($offer)) {
                    $item['attributes']['offer'] = $offer;
                }
            }

            if (is_array($item['attributes']['offer'])) {
                foreach ($item['attributes']['offer'] as $key => $value) { // убираем пустые значения из массива
                    if (!$value) {
                        unset($item['attributes']['offer'][$key]);
                    }
                }

                if ($product = Product::model()->findByPk($item['id'])) {
                    $values = $item['attributes']['offer'];
                    $item['attributes']['offer'] = '';
                    $offers = $product->getOffersData();

                    foreach ($offers['offers'] as $offer) {
                        $found = null;
                        if ((count($values) + 1) == count($offer)) {
                            foreach ($values as $prop => $val) {
                                if ($found === null) $found = true;
                                $found = $found && isset($offer[$prop]) && (trim((string)$offer[$prop]) === trim((string)$val));
                            }
                        }

                        if ($found) {
                            $item['price'] = $offer['price'] + $increasedSum;
                            break;
                        }
                    }

                    foreach ($values as $prop => $val) {
                        $item['attributes']['offer'] .= "<b>{$prop}</b>: {$val}<br/>";
                    }
                } else {
                    $item['attributes']['offer'] = '';
                }
