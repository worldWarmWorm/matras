...
<div class="form">
    ...
    $productOfferAttributes=$model->category->getOfferAttributes(true);
    if(!empty($productOfferAttributes)) {
        $tabs['Торговые предложения']=['content'=>$this->renderPartial('_form_product_offers', compact('model', 'form', 'productOfferAttributes'), true), 'id'=>'tab-offers'];
    }
    
    $tabs['Seo']=array('content'=>$this->renderPartial('_form_product_seo', compact('model', 'form'), true), 'id'=>'tab-seo');
