...
class HTools
{
    ...
    {
        return !(int)preg_replace("/[^0-9]/", "", $date); 
    }
     ...
     * @param boolean|array $isArray возвращать массив. 
    ...
    /**
     * Получить варианты комбинаций элементов
     * @param array $base массив базовых элементов (которые точно учитываются) 
     * вида array(value1, value2, ...). 
     * @param array $elements массив для формирования вариантов комбинаций,
     * из которых будут формироваться комбинации относительно базовых элементов.
     * @return array
     */
    public static function matrixVariants($base, $elements)
    {
        $variants=array();
        
        // формируем простую комбинацию относительно базовых элементов
        $tmpElements=$elements;
        while(($elm=array_shift($tmpElements))!==null) {
            $variant=array_merge($base, array($elm));
            $variants[]=$variant;
            $variants=array_merge($variants, static::matrixVariants($variant, $tmpElements));
        }
    ...
    /**
     * Получение всех комбинаций элеметов массива
     * @param [] $array входной массив значений
     * @return array
     */
    public static function matrix($array)
    {
        $matrix=array();
        while(($mainFixed=array_shift($array))!==null) {
            $forFixed=$array;
            $matrix[]=array($mainFixed);
            $matrix=array_merge($matrix, static::matrixVariants(array($mainFixed), $forFixed));
        }
        return $matrix;
    }
}
